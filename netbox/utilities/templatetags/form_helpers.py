...
def render_field_with_aria(field, has_helptext=None):
    ...
    #   1. <select> widgets hidden by Tom Select (ts-hidden-accessible,
    #      tabindex=-1), where scanners drop the <label for=> association. Mirror
    ...
    #   2. Fields rendered without a visible label (label='') — typically because
    #      a surrounding fieldset heading carries the name. Synthesize one from
    #      the field name so the control still has an accessible name. This is
    #      a last-resort fallback (English-only, reads poorly under non-English
    #      locales); in DEBUG we emit a warning so developers can supply a
    ...
    if 'aria-label' not in widget_attrs:
        if isinstance(field.field.widget, forms.Select) and field.label:
            ...
            if settings.DEBUG:
                ...
                warnings.warn(
                    f"Form field {form_name}.{field.name} has no label; using a "
                    "synthesized aria-label as a fallback. Set a translated label "
                    "on the field for proper accessibility.",
                    stacklevel=2,
                )
