...
class CustomFieldManager(models.Manager.from_queryset(RestrictedQuerySet)):
    ...
    def get_for_model(self, model, statuses=(CustomFieldStatusChoices.STATUS_ACTIVE,)):
        ...
        if custom_fields is None:
            ...
            custom_fields = list(
                self.get_queryset().filter(object_types=content_type).select_related('related_object_type')
            )
...
class CustomField(CloningMixin, ExportTemplatesMixin, OwnerMixin, ChangeLoggedModel):
    ...
    def _update_object_data(model, filters=None, commit_per_batch=False, **update_kwargs):
        ...
        :param commit_per_batch: Commit each batch independently rather than wrapping them all in a
            single transaction. Postgres holds a row lock on every row updated until the transaction
            commits, which for a job spanning a large table means blocking concurrent edits for its
            whole run, so the jobs commit as they go. Only for updates which can safely be resumed,
            and only outside an enclosing atomic block, which owns the commit regardless.
        """
        queryset = model.objects.filter(filters or Q())

        # Note that the filter is re-applied by each UPDATE, rather than resolved once when its
        # batch was selected, so a row which stopped qualifying in between is left alone. A value
        # assigned concurrently is therefore not overwritten in either mode.
        with nullcontext() if commit_per_batch else transaction.atomic():
            last_pk = 0
            while True:
                pks = list(
    ...
    def _exceeds_inline_limit(content_types):
        ...
        instead. The limit is CUSTOMFIELD_DATA_BATCH_SIZE objects across all of the given types.
        ...
        """
        remaining = CUSTOMFIELD_DATA_BATCH_SIZE
    ...
    def provision_data(self, object_types):
        ...
        Only the types passed are provisioned. A default reaches an object type's rows when the field
        is assigned to it and at no other time, as a default added to an existing field is documented
        ...
        (see CustomFieldStatusChoices). Objects created in the meantime are unaffected, as
        get_defaults_for_model() includes fields in that state.

        A field with no default has nothing to provision and goes live immediately, however many
        objects it applies to: an absent key reads the same as a null one, so writing one to every
        object would be an expensive no-op.
        """
    ...
    def populate_initial_data(self, content_types, commit_per_batch=False):
        ...
        The default is written unconditionally, so establishing that there is one to write falls to
        the caller; provision_data() is the gate which does so.
