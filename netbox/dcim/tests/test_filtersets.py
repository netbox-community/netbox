...
class DeviceTestCase(TestCase, ChangeLoggedFilterSetTests):
    ...
    def setUpTestData(cls):
        ...
        ipaddresses = (
            ...
            IPAddress(address='2001:db8::1/64', assigned_object=interfaces[1]),
            IPAddress(address='2001:db8::2/64', assigned_object=None),
        )
        IPAddress.objects.bulk_create(ipaddresses)
        Device.objects.filter(pk=devices[0].pk).update(primary_ip4=ipaddresses[0])
        Device.objects.filter(pk=devices[1].pk).update(primary_ip4=ipaddresses[1], primary_ip6=ipaddresses[2])
    ...
    def test_primary_ip4(self):
        ...
        params = {'primary_ip4_id': [addresses[0].pk]}
        self.assertEqual(self.filterset(params, self.queryset).qs.count(), 1)
        params = {'primary_ip4_id': [addresses[1].pk]}
        self.assertEqual(self.filterset(params, self.queryset).qs.count(), 1)
    ...
    def test_primary_ip6(self):
        ...
        self.assertEqual(self.filterset(params, self.queryset).qs.count(), 1)
        params = {'primary_ip6_id': [addresses[3].pk]}
