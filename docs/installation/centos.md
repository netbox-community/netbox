<h1>Getting Started on RHEL/Centos</h1>
...
NetBox requires the following RPMS:
...
```
# sudo yum install -y python-gunicorn supervisor
```
...
Save this configuration to `/etc/nginx/conf.d/netbox.conf`. Then, delete `/etc/nginx/defaul.d/default`
If there is no default file then continue anyways
...
Save the contents of the above example in `/etc/httpd/conf.d/netbox.conf`
On Centos the modules are loaded by default.
